...
class Engine(Component):
    ...
    stroke = UnitsFloat(78.8, io_direction='in', units='mm',
                        desc='Cylinder Stroke')
    bore = UnitsFloat(82.0, io_direction='in', units='mm', 
                      desc='Cylinder Bore')
    conrod = UnitsFloat(115.0, io_direction='in', units='mm', 
                        desc='Connecting Rod Length')
    comp_ratio = Float(9.3, io_direction='in', units=None, 
    ...
    n_cyl = Int(6, io_direction='in', desc = 'Number of Cylinders')
    ...
    RPM = UnitsFloat(1000.0, low=1000., high=6000., 
                     io_direction='in', units='1/min', 
                     desc='Engine RPM')
    throttle = Range(low=0.01, high=1.0, value=1.0, io_direction='in', 
                     desc='Throttle position (from low idle to wide open)')

    power = UnitsFloat(0., io_direction='out', units='kW', 
                       desc='Power at engine output')
    torque = UnitsFloat(0., io_direction='out', units='N*m', 
                        desc='Torque at engine output')
    fuel_burn = UnitsFloat(0., io_direction='out', units='l/s',
                           desc='Fuel Burn Rate')
    engine_weight = UnitsFloat(0., io_direction='out', units='kg', 
