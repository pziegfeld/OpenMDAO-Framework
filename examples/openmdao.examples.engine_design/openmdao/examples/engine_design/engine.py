...
class Engine(Component):
    ...
    stroke = UnitsFloat(78.8, io_direction='in', units='mm', desc='Cylinder Stroke')
    bore = UnitsFloat(82.0, io_direction='in', units='mm',  desc='Cylinder Bore')
    conrod = UnitsFloat(115.0, io_direction='in', units='mm',
                   desc='Connecting Rod Length')
    comp_ratio = Float(9.3, io_direction='in', desc='Compression Ratio')
    spark_angle = UnitsFloat(-37.0, io_direction='in', units='deg',
                    desc = 'Spark Angle with respect to TDC (Top Dead Center)')
    ncyl = Int(6, io_direction='in', desc = 'Number of Cylinders')
    IVO = UnitsFloat(11.0, io_direction='in', units='deg',
                     desc = 'Intake Valve Open before TDC (Top Dead Center)')
    IVC = UnitsFloat(53.0, io_direction='in', units='deg', 
                     desc = 'Intake Valve Open after BDC (Bottom Dead Center)')
    L_v = UnitsFloat(8.0, io_direction='in', units='mm',
                     desc='Maximum Valve Lift')
    D_v = UnitsFloat(41.2, io_direction='in', units='mm',
                     desc='Inlet Valve Diameter')

    RPM = UnitsFloat(1000.0, low=1000., high=6000., io_direction='in', 
                     units='1/min',  desc='Engine RPM')
    throttle = Range(low=0.01, high=1.0, value=1.0, io_direction='in', 
                     desc='Throttle position (from low idle to wide open)')

    power = UnitsFloat(0.0, io_direction='out', units='kW', 
                       desc='Power at engine output')
    torque = UnitsFloat(0.0, io_direction='out', units='N*m', 
                        desc='Torque at engine output')
    fuel_burn = UnitsFloat(0.0, io_direction='out', units='l/s', 
                           desc='Fuel Burn Rate')
    engine_weight = UnitsFloat(0.0, io_direction='out', units='kg', 
