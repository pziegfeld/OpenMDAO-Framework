...
class AxodComp(Component):
    ...
    input_filename = Str(io_direction='in')
    results = []

    hpower = Float(io_direction='out')
    ...
    tott  = Array(dtype=numpy.float32, shape=(48,), io_direction='out')
    totp  = Array(dtype=numpy.float32, shape=(48,), io_direction='out')
    mflow = Array(dtype=numpy.float32, shape=(48,), io_direction='out')
    effs  = Array(dtype=numpy.float32, shape=(48,), io_direction='out')
    effr  = Array(dtype=numpy.float32, shape=(48,), io_direction='out')
