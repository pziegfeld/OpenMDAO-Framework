...
class pyevolvedriver(Driver):
    ...
    objective = StringRef(io_direction='in',
                          desc='A string containing the objective function'
                               ' expression.')
    freq_stats = Int(0, io_direction='in')
    seed = Float(0., io_direction='in')
    population_size = Int(Consts.CDefGAPopulationSize, io_direction='in')
    
    sort_type = CBool(Consts.sortType["scaled"], io_direction='in',
                      desc='use Consts.sortType["raw"] or Consts.sortType["scaled"]') # can accept
    mutation_rate = Float(Consts.CDefGAMutationRate, io_direction='in')
    crossover_rate = Float(Consts.CDefGACrossoverRate, io_direction='in')
    generations = Int(Consts.CDefGAGenerations, io_direction='in')
    mini_max = CBool(Consts.minimaxType["minimize"], io_direction='in',
                    desc='use Consts.minimaxType["minimize"] or Consts.minimaxType["maximize"]')
    elitism = CBool(True, io_direction='in', desc='True of False')
    
    #outputs
    best_individual = Any(GenomeBase.GenomeBase(), io_direction='out')
